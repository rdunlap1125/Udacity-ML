    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            ...
            if agent_deadline <= self.hard_time_limit:
                self.done = True
                #print "Environment.step(): Primary agent hit hard time limit ({})! Trial aborted. total reward = {}, traffic penalties = {}, total penalties = {} ".format(self.hard_time_limit, agent.total_reward, agent.traffic_penalties, agent.total_penalties)
            elif self.enforce_deadline and agent_deadline <= 0:
                self.done = True
                #print "Environment.step(): Primary agent ran out of time! Trial aborted. total reward = {}, traffic penalties = {}, total penalties = {} ".format(agent.total_reward, agent.traffic_penalties, agent.total_penalties)
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                if state['deadline'] >= 0:
                    reward += 10  # bonus
                self.done = True
                #print "Environment.act(): Primary agent has reached destination in {} steps -- total reward = {}, traffic penalties = {}, total penalties = {}!".format(self.t, reward + agent.total_reward, agent.traffic_penalties, agent.total_penalties)  # [debug]
